CREATE TABLE [training].[TrainingEventInstructorAttachments]
(
    [TrainingEventInstructorAttachmentID] BIGINT IDENTITY(1,1) NOT NULL,
    [PersonID] BIGINT NOT NULL,
	[TrainingEventID] BIGINT NOT NULL, 
    [FileID] BIGINT NOT NULL, 
    [FileVersion] SMALLINT NOT NULL, 
	[TrainingEventInstructorAttachmentTypeID] INT NOT NULL,
    ...
    CONSTRAINT [PK_TrainingEventInstructorAttachments] 
		PRIMARY KEY ([TrainingEventInstructorAttachmentID]),
	CONSTRAINT [FK_TrainingEventInstructorAttachments_Persons] 
		FOREIGN KEY ([PersonID]) 
		REFERENCES [persons].[Persons]([PersonID]),
    CONSTRAINT [FK_TrainingEventInstructorAttachments_TrainingEvents] 
		FOREIGN KEY ([TrainingEventID]) 
		REFERENCES [training].[TrainingEvents]([TrainingEventID]),
	CONSTRAINT [FK_TrainingEventInstructorAttachments_Files]
		FOREIGN KEY (FileID)
		REFERENCES [files].[Files]([FileID]),
    CONSTRAINT [FK_TrainingEventInstructorAttachments_TrainingEventInstructorAttachmentTypes] 
		FOREIGN KEY ([TrainingEventInstructorAttachmentTypeID]) 
		REFERENCES [training].[TrainingEventInstructorAttachmentTypes]([TrainingEventInstructorAttachmentTypeID]),
	CONSTRAINT [FK_TrainingEventInstructorAttachments_AppUsers] 
        FOREIGN KEY ([ModifiedByAppUserID]) 
        REFERENCES [users].[AppUsers]([AppUserID])
)
ON [PRIMARY]
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [training].[TrainingEventInstructorAttachments_History]))
GO

/*  Table description  */
EXECUTE sp_addextendedproperty @name = N'MS_Description', 
	@level0type = N'SCHEMA', @level1type = N'TABLE', 
	@level0name = N'training', @level1name = N'TrainingEventInstructorAttachments',
...
EXEC sp_addextendedproperty @name = N'MS_Description',
    ...
    @level1type = N'TABLE',  @level1name = N'TrainingEventInstructorAttachments',
    @level2type = N'COLUMN', @level2name = N'TrainingEventInstructorAttachmentID',
...
EXEC sp_addextendedproperty @name = N'MS_Description',
    @level0type = N'SCHEMA', @level0name = N'training',
    @level1type = N'TABLE',  @level1name = N'TrainingEventInstructorAttachments',
    @level2type = N'COLUMN', @level2name = N'TrainingEventInstructorAttachmentTypeID',
	@value = N'Indicates the type of TrainingEventInstructor document.  Foreign key pointing to the TrainingEventInstructorAttachmentTypes table.'
GO
