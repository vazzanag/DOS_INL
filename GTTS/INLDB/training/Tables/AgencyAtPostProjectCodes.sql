/*
    **************************************************************************
    AgencyAtPostProjectCodes.sql
    **************************************************************************    
*/      
CREATE TABLE [training].[AgencyAtPostProjectCodes] 
(
    [PostID] INT NOT NULL, 
    ...
    CONSTRAINT [PK_AgencyAtPostProjectCodes]
        PRIMARY KEY ([PostID], [ProjectCodeID]),
    CONSTRAINT [FK_AgencyAtPostProjectCodes_Posts]
        FOREIGN KEY ([PostID]) 
        REFERENCES [location].[Posts]([PostID]),
	CONSTRAINT [FK_AgencyAtPostProjectCodes_UnitLibrary]
		FOREIGN KEY ([UnitID]) 
		REFERENCES [unitlibrary].[Units]([UnitID]),
    CONSTRAINT [FK_AgencyAtPostProjectCodes_ProjectCodes]
        FOREIGN KEY ([ProjectCodeID]) 
        REFERENCES [training].[ProjectCodes]([ProjectCodeID]),
    CONSTRAINT [FK_AgencyAtPostProjectCodes_AppUsers] 
        FOREIGN KEY ([ModifiedByAppUserID]) 
        REFERENCES [users].[AppUsers]([AppUserID])
)
ON [PRIMARY]
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [training].[AgencyAtPostProjectCodes_History]))
GO        
      
/*  Table description  */
EXECUTE sp_addextendedproperty @name = N'MS_Description', 
	@level0type = N'SCHEMA', @level0name = N'training',
	@level1type = N'TABLE',  @level1name = N'AgencyAtPostProjectCodes',
	@value = N'A subset of Project Codes used by a specific Agency/Unit at a Post.  Agency/Unit administrators at a Post have the ability to designate what Project Codes are allowed to be used by their Agency/Unit users at the Post.'
GO
