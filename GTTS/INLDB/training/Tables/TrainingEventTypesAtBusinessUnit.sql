CREATE TABLE [training].[TrainingEventTypesAtBusinessUnit]
(
    ...
    CONSTRAINT [PK_TrainingEventTypesAtBusinessUnit]
        PRIMARY KEY ([BusinessUnitID], [TrainingEventTypeID]),
    CONSTRAINT [FK_TrainingEventTypesAtBusinessUnit_BusinessUnit]
        FOREIGN KEY ([BusinessUnitID]) 
        REFERENCES [users].[BusinessUnits]([BusinessUnitID]),
    CONSTRAINT [FK_TrainingEventTypesAtBusinessUnit_TrainingEventType]
        FOREIGN KEY ([TrainingEventTypeID]) 
        REFERENCES [training].[TrainingEventTypes]([TrainingEventTypeID]),        
    CONSTRAINT [FK_TrainingEventTypesAtBusinessUnit_AppUsers] 
        FOREIGN KEY ([ModifiedByAppUserID]) 
        REFERENCES [users].[AppUsers]([AppUserID])
)
ON [PRIMARY]
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [training].[TrainingEventTypesAtBusinessUnit_History]))
GO  

/*  Table description  */
EXECUTE sp_addextendedproperty @name = N'MS_Description', 
	@level0type = N'SCHEMA', @level0name = N'training',
	@level1type = N'TABLE',  @level1name = N'TrainingEventTypesAtBusinessUnit',
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Second part of the Primary key for this table.  Foreign key pointing to the TrainingEventTypes table.'
GO
