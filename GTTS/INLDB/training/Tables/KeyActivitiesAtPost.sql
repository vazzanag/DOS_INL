CREATE TABLE [training].[KeyActivitiesAtPost]
(
    [PostID] INT NOT NULL, 
    ...
    CONSTRAINT [PK_KeyActivitiesAtPost]
        PRIMARY KEY ([PostID], [KeyActivityID]),
    CONSTRAINT [FK_KeyActivitiesAtPost_Posts]
        FOREIGN KEY ([PostID]) 
        REFERENCES [location].[Posts]([PostID]),
    CONSTRAINT [FK_KeyActivitiesAtPost_KeyActivities]
        FOREIGN KEY ([KeyActivityID]) 
        REFERENCES [training].[KeyActivities]([KeyActivityID]),        
    CONSTRAINT [FK_KeyActivitiesAtPost_AppUsers] 
        FOREIGN KEY ([ModifiedByAppUserID]) 
        REFERENCES [users].[AppUsers]([AppUserID])
)
ON [PRIMARY]
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [training].[KeyActivitiesAtPost_History]))
GO  

/*  Table description  */
EXECUTE sp_addextendedproperty @name = N'MS_Description', 
	@level0type = N'SCHEMA', @level0name = N'training',
	@level1type = N'TABLE',  @level1name = N'KeyActivitiesAtPost',
    @value = N'A subset of Key Activities used at a specific Post.  Post administrators have the ability to designate which Key Activities are allowed to be used by users at the Post.'
GO
