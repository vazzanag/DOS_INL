/*
    NAME:   NewPostParticipants

    DESCR:  This is the import migration table that receives the imported records
            from the [ImportParticpants] table.

            The data is initially imported from the xlsx file into the [ImportParticpants] 
            table and then appended into the [NewPostParticipants] table (this table 
...
*/
CREATE TABLE [migration].[NewPostParticipants]
(
	[NewPostParticipantID] BIGINT IDENTITY (1, 1) NOT NULL, -- Internal PK of table.    

	[NewPostImportID] BIGINT NULL,							-- FK to [NewPostImportLog] table.   

    [ParticipantStatus] NVARCHAR(15) NULL,					-- Indicates if the Participant is an Event Participant or an Event Alternate.  This value
    [ParticipantFlag] BIT DEFAULT '0' NULL,                 -- is converted to a boolean and written to [ParticipantFlag] as "Participant" = TRUE or 
                                                            -- "Alternate" = FALSE.  When the data is written to the GTTS data tables, [ParticipantFlag]
                                                            -- is mapped to [TrainingEventParticipants].[IsParticipant] column.

    -- *****************************************************
    -- BEGIN [Persons] BLOCK
    -- *****************************************************
	[FirstMiddleName] NVARCHAR(200) NULL,					-- The Participant's Given & Middle Name(s).  
                                                            -- Mapped to [Persons].[FirstMiddleNames].

	[LastName] NVARCHAR(50) NULL,							-- The Participant's Family Name.  
                                                            -- Mapped to [Persons].[LastNames].

	[NationalID] NVARCHAR(100) NULL,                        -- The Participant's National ID (if applicable in their country).  
                                                            -- Mapped to [Persons].[NationalID].

	[Gender] NVARCHAR(10) NULL,                             -- The Participant's gender or sex.  This value is converted to single character string
	...
	[IsUSCitizen] NVARCHAR(3) NULL,							-- Indicates if the Participant is a US Citizen.  This value is converted to a boolean
	...
	[DOB] DATETIME NULL,                                    -- Participant's Date of Birth in MM/DD/YYYY format.  
                                                            -- Mapped to [Persons].[DOB].

	[POBCity] NVARCHAR(100) NULL,                           -- Name of the City that the Participant was born in.
    ...
    [POBState] NVARCHAR(75) NULL,                           -- Name of the State that the Participant was born in.
	...
	[POBCountry] NVARCHAR(75) NULL,                         -- Name of the Country that the Participant was born in.
	...
	[ResidenceCity] NVARCHAR(100) NULL,                     -- Name of the City that the Participant resides in.
    ...
    [ResidenceState] NVARCHAR(75) NULL,                     -- Name of the State that the Participant resides in.
	...
	[ResidenceCountry] NVARCHAR(75) NULL,                   -- Name of the Country that the Participant resides in.
	...
	[ContactEmail] NVARCHAR(256) NULL,                      -- Participant's Email Address.  
                                                            -- Mapped to [Persons].[ContactEmail].
                                                            -- ????????????????????????????????????????????????????????????????????
                                                            -- MAPPING CHANGED TO [PersonsUnitLibraryInfo] ==> WHEN ?????
                                                            -- ???????????????????????????????????????????????????????????????????? 

	[ContactPhone] NVARCHAR(50) NULL,                       -- The Participant's Telephone Number & Extension (if applicable).  
                                                            -- Mapped to [Persons].[ContactPhone].
                                                            -- ????????????????????????????????????????????????????????????????????
                                                            -- MAPPING CHANGED TO [PersonsUnitLibraryInfo] ==> WHEN ?????
                                                            -- ???????????????????????????????????????????????????????????????????? 

	[HighestEducation] NVARCHAR(25) NULL,                   -- The highest level of education the Participant has completed. 
	...
	[EnglishLanguageProficiency] NVARCHAR(50) NULL,         -- The level of proficiency the Participant has in the English Language. 
    ...
    -- *****************************************************
    [UnitGenID]	NVARCHAR(50) NULL,                          -- The UnitGenID value that identifies the Participant's assigned Unit.  
                                                            ...
                                                            -- identified in the Participant's current [PersonsUnitLibraryInfo] record.

	[UnitBreakdown] NVARCHAR(2048) NULL,                    -- Identifies the Participant's assigned Unit.
    ...
    [UnitID] BIGINT NULL,                                   -- Identifies the Participant's assigned Unit.   Refer to comments above for 
	...
	[JobTitle] NVARCHAR(1024) NULL,							-- The Participant's Job or Position Title.
                                                            -- Mapped to [PersonsUnitLibraryInfo].[JobTitle]

    [Rank] NVARCHAR(100) NULL,                              -- The name of the Participant's Rank (if applicable).
	...
	[UnitCommander] NVARCHAR(3) NULL,						-- Indicates if the Participant is a Unit Commander.   
	...
	[YearsInPosition] INT NULL,                             -- The number of years that the Participant has held the position indicated by [JobTitle].
                                                            -- Mapped to [PersonsUnitLibraryInfo].[YearsInPosition].

	[PoliceMilSecID] NVARCHAR(100) NULL,                    -- The Participant's Military, Police, or Security Services Badge or ID Number.
                                                            -- Mapped to [PersonsUnitLibraryInfo].[PoliceMilSecID].

	[POCName] NVARCHAR(200) NULL,                           -- The name of the Participant's Unit Point of Contact.
                                                            -- Mapped to [Units].[POCName].

	[POCEmailAddress] NVARCHAR(256) NULL,                   -- The Email Address for the Participant's Unit Point of Contact.
                                                            -- Mapped to [Units].[POCEmailAddress]. 
    -- *****************************************************
    -- END [PersonsUnitLibraryInfo] & [Units] BLOCK
    -- *****************************************************

	[VettingType] NVARCHAR(75) NULL,                        -- Indicates the type of vetting that the Participant should be undergoing.
    ...
    [IsLeahyVettingReq] BIT NULL,                           -- columns as indicated below:
                                                            ...
                                                            -- These columns will then be mapped to the [PersonsUnitLibraryInfo].[IsVettingRequired] &
                                                            -- [PersonsUnitLibraryInfo].[IsLeahyVettingReq] columns.
	...
	[DepartureCity]	NVARCHAR(127) NULL,                     -- Name of the City that the Participant is departing from.
                                                            ...
                                                            -- ????????????????????????????????????????????????????????????????????
    [DepartureCityID] INT NULL,                             -- Mapped to [TrainingEventParticipants].[DepartureCityID] column.

	[PassportNumber] NVARCHAR(100) NULL,                    -- The Participant's current passport identification number.  
                                                            -- This value will be mapped to [Persons].[PassportNumber].
                                                            -- ????????????????????????????????????????????????????????????????????
                                                            -- MAPPING CHANGED TO [PersonsUnitLibraryInfo] ==> WHEN ?????
                                                            -- ???????????????????????????????????????????????????????????????????? 

	[PassportExpirationDate] DATETIME NULL,                 -- The date that the Participant's current passport expires in MM/DD/YYYY format.
                                                            -- This value will be mapped to [Persons].[PassportExpirationDate].
                                                            -- ????????????????????????????????????????????????????????????????????
                                                            -- MAPPING CHANGED TO [PersonsUnitLibraryInfo] ==> WHEN ?????
                                                            -- ????????????????????????????????????????????????????????????????????

	[Comments] NVARCHAR(4000) NULL,                         -- Comments or notes regarding the Participant.  
    ...
    [PersonID] BIGINT NULL,                                 -- [Persons].[PersonID] & [TrainingEventParticipants].[PersonID] 
                                                            ...
                                                            -- of the import of the Participant.  Foreign key pointing to the [Persons] table and also the
                                                            -- [TrainingEventParticipants] table.
    
    [PersonExists] BIT DEFAULT '0' NULL,                    -- Flag that indicates if the Participant is new to the GTTS database (FALSE = 0) 
    ...
    [TrainingEventID] BIGINT NULL,                          -- [TrainingEventID] value of the new [TrainingEvents] table record that was created as a result
                                                            -- of the import of the Training Event that this Participant is participating in.
	...
	[ImportStatus] NVARCHAR(100) NULL,						-- Status of the import process.  Valid values are:
                                                            -- 'Uploaded'                 = Data was uploaded into the [NewPostParticipants] table.
    ...
    CONSTRAINT [PK_NewPostParticipants] 
		PRIMARY KEY ([NewPostParticipantID]),

    CONSTRAINT [FK_NewPostParticipants_NewPostImportLog] 
        FOREIGN KEY ([NewPostImportID]) 
        REFERENCES [migration].[NewPostImportLog]([NewPostImportID]),

    CONSTRAINT [FK_NewPostParticipants_Persons] 
        FOREIGN KEY ([PersonID]) 
        REFERENCES [persons].[Persons]([PersonID]),

    CONSTRAINT [FK_NewPostParticipants_TrainingEvents] 
        FOREIGN KEY ([TrainingEventID]) 
        REFERENCES [training].[TrainingEvents]([TrainingEventID]),

    CONSTRAINT [FK_NewPostParticipants_AppUsers] 
        FOREIGN KEY ([ModifiedByAppUserID]) 
        REFERENCES [users].[AppUsers]([AppUserID])
)
ON [PRIMARY]
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = [migration].[NewPostParticipants_History]))
GO

/*  Table description  */
EXECUTE sp_addextendedproperty @name = N'MS_Description', 
	@level0type = N'SCHEMA', @level0name = N'migration',
	@level1type = N'TABLE',  @level1name = N'NewPostParticipants',
...
EXEC sp_addextendedproperty @name = N'MS_Description',
    @level0type = N'SCHEMA', @level0name = N'migration',
    @level1type = N'TABLE',  @level1name = N'NewPostParticipants',
    @level2type = N'COLUMN', @level2name = N'NewPostParticipantID',
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates if the Participant is an Event Participant or Event Alternate.  This value is converted to a boolean and written to [ParticipantFlag] as "Participant" = TRUE or "Alternate" = FALSE.'
GO

EXEC sp_addextendedproperty @name = N'MS_Description',
    @level0type = N'SCHEMA', @level0name = N'migration',
    @level1type = N'TABLE',  @level1name = N'NewPostParticipants',
    @level2type = N'COLUMN', @level2name = N'ParticipantFlag',
	@value = N'Indicates if the Participant is an Event Participant or Event Alternate.  "Participant" = TRUE or "Alternate" = FALSE.  This column is mapped to [TrainingEventParticipants].[IsParticipant].'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The Participant''s gender or sex.  This value is converted to single character string and written to [GenderFlag] as "M" for "Male" or "F" for "Female".'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The Participant''s gender or sex.  "M" for "Male" or "F" for "Female".  This column is mapped to mapped to [Persons].[Gender].'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates if the Participant is a US Citizen.  This value is converted to a boolean and written to [USCitizenFlag] as "Yes" = TRUE or "No" = FALSE.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates if the Participant is a US Citizen.  "Yes" = TRUE or "No" = FALSE.  This column is mapped to mapped to [Persons].[IsUSCitizen].'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N' LocationID from the [Locations] table for the Participant''s Residence location.  This value requires that a valid [ResidenceCity], [ResidenceState], & [ResidenceCountry] exist in the database.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'If the Unit Library for the Host Nation is in the [Units] table and the [UnitGenID] value for the Unit is known, then it will be in this column.  Otherwise this column will be NULL.  Refer to the [Units] table for details on how the [UnitGenID] is created.  This value will be used to validate the Participant''s Unit information against the UnitLibrary for the Host Nation.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Identifies the Participant''s assigned Unit. This value is validated against the [Units] table.  If the Unit specified by [UnitGenID] and/or [UnitBreakdown] exists, the Unit''s UnitID value is stored in this column.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The Participant''s Job or Position Title.  This column is mapped to [PersonsUnitLibraryInfo].[JobTitle]'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates if the Participant is a Unit Commander.  This value is converted to a boolean and written to [UnitCommanderFlag] as "Yes" = TRUE or "No" = FALSE.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates if the Participant is a Unit Commander.  Yes" = TRUE or "No" = FALSE.  This column is mapped to mapped to [PersonsUnitLibraryInfo].[IsUnitCommander].'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The number of years that the Participant has held the position indicated by [JobTitle].  Mapped to [PersonsUnitLibraryInfo].[YearsInPosition].'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The Participant''s Military, Police, or Security Services Badge or ID Number.  Mapped to [PersonsUnitLibraryInfo].[PoliceMilSecID].'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates the type of vetting that the Participant should be undergoing.  This value will be mapped to both [IsVettingRequired] and[IsLeahyVettingReq] as follows:  NONE ==> ([IsVettingRequired] = NO & [IsLeahyVettingReq] = NO), COURTESTY ==> ([IsVettingRequired] = YES & [IsLeahyVettingReq] = NO), or LEHY ==> ([IsVettingRequired] = YES & [IsLeahyVettingReq] = YES)  These are boolean columns where "YES" = TRUE, "NO" = FALSE.'  
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Result of validation of the [NewPostParticipants].[VettingType] column.  Mapped to [Persons].[IsVettingRequired].'  
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Result of validation of the [NewPostParticipants].[VettingType] column.  Mapped to [Persons].[IsLeahyVettingReq].'  
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Indicates if the Participant underwent an external vetting process not covered by the [HasGovTrust] flag.  Mapped to [TrainingEventParticipants].[OtherVettinglag].'  
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The [PersonID] value of the new Person record that was created (or matched to) as a result of the import of the Participant.  Foreign key pointing to the [Persons] table and also the [TrainingEventParticipants] table.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Flag that indicates if the Participant is new to the GTTS database (FALSE = 0) or already exists in the [Persons] table (TRUE = 1).'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'The [TrainingEventID] value of the new [TrainingEvents] table record that was created as a result of the import of the Training Event that this Participant is participating in.  Foreign key pointing to the [TrainingEvents] table.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Contains the results of the validations processing during the upload process.  This is an internal management & control column that does not have a matching database data column outside of the [NewPostParticipants] table.'
GO
...
EXEC sp_addextendedproperty @name = N'MS_Description',
	...
	@value = N'Identifies the version number of the template spreadsheets.  This is an internal management & control column that does not have a matching database data column outside of the [NewPostParticipants] table.'
GO
