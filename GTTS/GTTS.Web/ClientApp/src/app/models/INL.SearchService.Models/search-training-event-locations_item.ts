

import { ISearchTrainingEventLocations_Item } from './isearch-training-event-locations_item';

export class SearchTrainingEventLocations_Item implements ISearchTrainingEventLocations_Item {
  
	public LocationID: number = 0;
	public LocationName: string = "";
	public IsActive: boolean = false;
	public ModifiedByAppUserID: number = 0;
	public ModifiedDate: Date = new Date(0);
	public AddressLine1: string = "";
	public AddressLine2: string = "";
	public AddressLine3: string = "";
	public CityID: number = 0;
	public CityName: string = "";
	public StateID: number = 0;
	public StateName: string = "";
	public StateCodeA2: string = "";
	public StateAbbreviation: string = "";
	public StateINKCode: string = "";
	public CountryID: number = 0;
	public CountryName: string = "";
	public GENCCodeA2: string = "";
	public CountryAbbreviation: string = "";
	public CountryINKCode: string = "";
	public Latitude: string = "";
	public Longitude: string = "";
  
}


