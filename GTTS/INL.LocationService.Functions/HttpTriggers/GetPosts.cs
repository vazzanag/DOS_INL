...
namespace INL.LocationService.Functions.HttpTriggers
{
	public static class GetPosts
	{
		...
		public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "posts")]HttpRequestMessage request, ILogger log, Microsoft.Azure.WebJobs.ExecutionContext context)
		{
			using (log.BeginScope(context.InvocationId.ToString()))
			{
				...
				try
				{
					...
					using (var sqlConnection = await helper.GetSqlConnectionAsync())
					{
						...
						result = locationService.GetPosts();
					}
