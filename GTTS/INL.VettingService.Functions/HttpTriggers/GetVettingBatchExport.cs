...
{
    public static class GetVettingBatchExport
    {
        [FunctionName("GetVettingBatchExport")]
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "batches/{vettingBatchID}/export")]HttpRequestMessage request,
            long vettingBatchID, ILogger log, ExecutionContext context)
        {
            using (log.BeginScope(context.InvocationId.ToString()))
            {
                ...
                try
                {
                    ...
                    using (var sqlConnection = await helper.GetSqlConnectionAsync())
                    {
                        ...
                        result = vettingService.ExportVettingBatch(vettingBatchID, null, trainingServiceClient);
                    }
